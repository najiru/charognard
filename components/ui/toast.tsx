...
function ToastProvider({
  ...
  return (
    <Toast.Provider toastManager={toastManager} {...props}>
      {children}
      <Toasts position={position} container={container} />
    </Toast.Provider>
  );
}

function Toasts({ position = "bottom-right", container }: { position: ToastPosition; container?: HTMLElement }) {
  const { toasts } = Toast.useToastManager();
  const isTop = position.startsWith("top");

  return (
    <Toast.Portal data-slot="toast-portal" container={container}>
      <Toast.Viewport
        className={cn(
          "fixed z-10001 mx-auto flex w-[calc(100%-var(--toast-inset)*2)] max-w-90 [--toast-inset:--spacing(4)] sm:[--toast-inset:--spacing(8)]",
    ...
    <Toast.Portal data-slot="toast-portal-anchored">
      ...
      >
        {toasts.map((toast) => {
          ...
          return (
            <Toast.Positioner
              className="z-10001 max-w-[min(--spacing(64),var(--available-width))]"
